...
if __name__ == "__main__":
    ...
    if os.path.exists(csv_file):
        ...
        combined_df.to_csv("fake_job_postings.csv", index=False)
        print(f"Success! Augmented data SAVED to '{csv_file}' for ML consumption.")
        print(f"New total rows: {len(combined_df)}")
    else:
        print(f"Error: {csv_file} not found. Saving new data to 'fake_job_postings.csv'")
        new_data.to_csv("fake_job_postings.csv", index=False)
